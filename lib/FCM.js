...
export default function FCM(args, pushType) {
  if (typeof args !== 'object' || !args.firebaseServiceAccount) {
    throw new Parse.Error(Parse.Error.PUSH_MISCONFIGURED, 'FCM Configuration is invalid');
  }
...
FCM.prototype.send = function (data, devices) {
  ...
  const sendToDeviceSlice = (deviceSlice, pushType) => {
    ...
    const fcmPayload = generateFCMPayload(data, pushId, timestamp, deviceTokens, pushType);
    const length = deviceTokens.length;
    log.info(LOG_PREFIX, `sending push to ${length} devices`);

    return this.sender.sendEachForMulticast(fcmPayload.data).then(response => {
      const promises = [];
      const failedTokens = [];
  ...
  const allPromises = Promise.all(slices.map(slice => sendToDeviceSlice(slice, this.pushType))).catch(err => {
...
function _APNSToFCMPayload(requestData) {
  ...
  let expirationTime = requestData['expiration_time'] || coreData['expiration_time'];
...
function payloadConverter(requestData, pushType, timeStamp) {
  ...
  if (pushType === 'apple') {
    ...
    throw new Parse.Error(Parse.Error.PUSH_MISCONFIGURED, 'Unsupported push type, apple or android only.');
  }
}
